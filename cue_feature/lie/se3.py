...
import torch
from lie.sinc import sinc1, sinc2, sinc3
from lie import so3
...
def mat(x):
    ...
    X = torch.stack((torch.stack((O, -w3, w2, v1), dim=1), torch.stack((w3, O, -w1, v2), dim=1), torch.stack((-w2, w1, O, v3), dim=1), torch.stack((O, O, O, O), dim=1)), dim=1)
