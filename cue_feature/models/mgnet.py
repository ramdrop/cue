...
class MGNet2(ME.MinkowskiNetwork):
    NORM_TYPE = None
    BLOCK_NORM_TYPE = 'BN'
    ...
    def __init__(self, in_channels=3, out_channels=32, bn_momentum=0.1, normalize_feature=None, conv1_kernel_size=None, D=3):
        ...
        # ------------------------- sigma-p prediction head ------------------------ #
        # 96 > 64
        self.sigma_conv1_tr_p = ME.MinkowskiConvolution(in_channels=CHANNELS[1] + TR_CHANNELS[2], out_channels=TR_CHANNELS[1], kernel_size=1, stride=1, dilation=1, bias=False, dimension=D)
        # 64 > 1
        self.sigma_final_p = ME.MinkowskiConvolution(in_channels=TR_CHANNELS[1], out_channels=1, kernel_size=1, stride=1, dilation=1, bias=True, dimension=D)
        self.sigma_final_p.kernel = torch.nn.parameter.Parameter(torch.zeros_like(self.sigma_final.kernel))
        self.sigma_final_p.bias = torch.nn.parameter.Parameter(torch.log(torch.tensor([1e-3]))) # NOTE: initialize as suggested
    ...
    def forward(self, x):              # ([N, 1])
        ...
        # MU BRANCH ================== #
        out = MEF.relu(self.conv1_tr(out_shared))          # ([N, 64])
        ...
        # SIGMA BRANCH =============== #
        sigma_out = MEF.relu(self.sigma_conv1_tr(out_shared))                  # ([N, 64])
        sigma_out = MEF.softplus(self.sigma_final(sigma_out))                  # ([N, 1])

        # SIGMA_P BRANCH ============= #
        sigma_p_out = MEF.relu(self.sigma_conv1_tr_p(out_shared))                  # ([N, 64])
        sigma_p_out = MEF.softplus(self.sigma_final_p(sigma_p_out))                  # ([N, 1])

        sigma_tol_out = sigma_out + sigma_p_out

        return mu_out, sigma_tol_out


class MGNetBN2(MGNet2):
    NORM_TYPE = 'BN'


class MGNetBN2B(MGNet2):
    NORM_TYPE = 'BN'
    CHANNELS = [None, 32, 64, 128, 256]
    TR_CHANNELS = [None, 64, 64, 64, 64]


class MGNetBN2C(MGNet2):
    NORM_TYPE = 'BN'
    CHANNELS = [None, 32, 64, 128, 256]
    TR_CHANNELS = [None, 64, 64, 64, 128]


class MGNetBN2D(MGNet2):
    NORM_TYPE = 'BN'
    CHANNELS = [None, 32, 64, 128, 256]
    TR_CHANNELS = [None, 64, 64, 128, 128]


class MGNetBN2E(MGNet2):
    NORM_TYPE = 'BN'
    CHANNELS = [None, 128, 128, 128, 256]
    TR_CHANNELS = [None, 64, 128, 128, 128]


class MGNetIN2(MGNet2):
    NORM_TYPE = 'BN'
    BLOCK_NORM_TYPE = 'IN'


class MGNetIN2B(MGNetBN2B):
    NORM_TYPE = 'BN'
    BLOCK_NORM_TYPE = 'IN'


class MGNetIN2C(MGNetBN2C):
    NORM_TYPE = 'BN'
    BLOCK_NORM_TYPE = 'IN'


class MGNetIN2D(MGNetBN2D):
    NORM_TYPE = 'BN'
    BLOCK_NORM_TYPE = 'IN'


class MGNetIN2E(MGNetBN2E):
    NORM_TYPE = 'BN'
    BLOCK_NORM_TYPE = 'IN'
